...
## 🛡️ Mutation Invalidation & Variant Consistency

Materialized variants are invalidated using the same **Route Cache Epoch** mechanism as raw cache entries, ensuring total consistency across all projections after an upstream mutation.

### 1. Global Route Invalidation

When a successful mutation occurs (e.g., via a `PATCH`, `PUT`, or `DELETE` request with an explicit invalidation policy), the **Route Cache Epoch** for the target route is incremented in the global epoch store.

- **Key Embedding**: Every variant key generated by `build_cache_key()` embeds the current epoch:
  `var:GET|{route_id}|epoch:{N}|{path}|...`
- **Instant Invalidation**: As soon as the epoch increments from `N` to `N+1`, all variant keys generated for the route change immediately. This simultaneously invalidates the raw response and **all** its materialized variants.
- **Zero-Scan Cleanup**: Because invalidation is logical (via the epoch), no scan of field projections or variant signatures is required during the mutation hot path. Old variants for epoch `N` remain in storage but become unreachable. They are eventually evicted by the cache's native ring-buffer advancement or TTL expiry.

### 2. Physical Removal (Optional)
While epoch-based invalidation provides logical consistency, exact variant removal is also possible:
*   **`l1_remove(cache, variant_key)`**: Can be used if a specific variant key is known (e.g., during targeted background cleanup).
*   **Namespace Isolation**: Because variants are isolated under the `var:` namespace, they do not interfere with raw cache invalidation primitives.

### 3. Admission Consistency
The **Variant Admission Registry** (heat accumulation) is reset upon process restart or route reconfiguration. While heat survives epoch bumps in the current version (v1), the materialization gating ensures that only "hot" variants are re-cached under the new epoch, preventing cache pollution from stale permutations.
