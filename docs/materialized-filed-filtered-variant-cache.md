...
When a mutation occurs (e.g., via a `POST/PUT` request with `mutation: full` or an explicit invalidation event), the **Route Cache Epoch** is incremented in the global epoch store.
*   **Key Embedding**: Every variant key generated by `build_cache_key()` embeds the current epoch:
    `var:GET|{route_id}|epoch:{N}|{path}|...`
*   **Instant Invalidation**: As soon as the epoch increments from `N` to `N+1`, all variant keys generated for the route change immediately.
*   **Zero-Scan Cleanup**: Old variants for epoch `N` remain in storage but become unreachable via standard lookup paths. They are eventually evicted by the cache's native FIFO/LRU mechanisms or TTL expiry.
