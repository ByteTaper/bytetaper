    ...
    for (const auto& route : policy.routes) {
        ...
        if (route.cache.enabled || route.cache.behavior != TqCacheBehavior::Default) {
            ...
            if (route.cache.enabled || route.cache.ttl_ms > 0) {
                out += "      ttl_seconds: " + std::to_string(route.cache.ttl_ms / 1000u) + "\n";
            }
            if (route.cache.l1.enabled || route.cache.l1.capacity_entries > 0 ||
                route.cache.l2.enabled || !route.cache.l2.path.empty()) {
                out += "      layers:\n";
                if (route.cache.l1.enabled || route.cache.l1.capacity_entries > 0) {
                    out += "        l1:\n";
                    out += "          enabled: " +
                           std::string(route.cache.l1.enabled ? "true" : "false") + "\n";
                    out += "          capacity_entries: " +
                           std::to_string(route.cache.l1.capacity_entries) + "\n";
                }
                if (route.cache.l2.enabled || !route.cache.l2.path.empty()) {
                    out += "        l2:\n";
                    out += "          enabled: " +
                           std::string(route.cache.l2.enabled ? "true" : "false") + "\n";
                    out += "          path: " + route.cache.l2.path + "\n";
                }
            }
            if (route.cache.private_cache.enabled) {
                out += "      private_cache: true\n";
            }
            ...
            if (route.cache.field_variant.enabled) {
                ...
                out +=
                    "        ttl_max_ms: " + std::to_string(route.cache.field_variant.ttl_max_ms) +
                    "\n";
            }
            ...
            if (route.cache.invalidation.enabled) {
                ...
                out += "        success_status:\n";
                out += "          min: " +
                       std::to_string(route.cache.invalidation.success_status_min) + "\n";
                out += "          max: " +
                       std::to_string(route.cache.invalidation.success_status_max) + "\n";
        ...
        if (route.compression.enabled) {
            ...
            out += "      enabled: true\n";
            out +=
                "      min_size_bytes: " + std::to_string(route.compression.min_size_bytes) + "\n";
        ...
        if (route.coalescing.enabled) {
            ...
            out +=
                ...
                "\n";
            out +=
                "      handoff_buffer_ms: " + std::to_string(route.coalescing.handoff_buffer_ms) +
                "\n";
            out += "      result_ready_retention_ms: " +
                   std::to_string(route.coalescing.result_ready_retention_ms) + "\n";
            out += "      max_waiters_per_key: " +
                   std::to_string(route.coalescing.max_waiters_per_key) + "\n";
